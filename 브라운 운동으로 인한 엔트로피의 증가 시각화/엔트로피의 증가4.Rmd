...
for(i in 1:1200){ # i가 1, 21, 41, 61, 81 같이 될 때, 즉 처음 방향을 정한 다음 원자들이 20번씩 이동할 때마다 이동 위치가 바뀌게 함 
  ...
  scatter3D(dc$x, dc$y, dc$z, colvar= dc$Temp, col= c("blue", "red"), theta= 20, phi=30, pch= 19, cex=1.1, bty = "u", col.panel ="#005B00", col.grid = "#005000",
            ...
            sub= paste0("* 파란색: 온도 낮음, 빨간색: 온도 높음                                                                           총 원자 수: 30   왼쪽 방 원자 수:  ", le_space, le, "   오른쪽 방 원자 수:  ", ri_space, ri, "       "))
  ...
  if(i%%20==1){
    ...
    repeat{ # 원자들의 좌표가 무조건 겹칠 수밖에 없다면 원자 1부터 다시 새로운 위치를 정해주는 기능
      ...
      for(j in 1:30){ # 각 원자들이 0.05초 동안 움직여서 나온 새로운 좌표를 기존의 데이터 프레임에 추가해주는 기능 (원자는 총 40개)
