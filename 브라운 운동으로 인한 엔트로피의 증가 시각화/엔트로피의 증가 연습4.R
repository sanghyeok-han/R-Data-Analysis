...
repeat{ # 처음 분자들의 좌표를 정해주는 기능
  ...
  dc<- data.frame(particle= 1:30, Temp= rep(c(1,2), each= 15), num= 0, x= sample(1:5,30, replace= T), y= sample(1:10,30, replace= T),
                  z= sample(1:10,30, replace= T), delx= 0, dely= 0, delz= 0, mc= 0, om= -1) # dc: dots' coordinate, mc: move count, om: object move  # Temp의 1: low, 2: high
...
for(i in 1:20){ 
  ...
  scatter3D(dc$x, dc$y, dc$z, colvar= dc$Temp, col= c("blue", "red"), theta= 20, phi=30, pch= 19, cex=1.1, bty = "u", col.panel ="#005B00", col.grid = "#005000",
            ...
            sub= paste0("* 파란색: 온도 낮음, 빨간색: 온도 높음                                                                           총 분자 수: 30   왼쪽 방 분자 수:  ", le_space, le, "   오른쪽 방 분자 수:  ", ri_space, ri, "       "))
  
  dc2<- data.frame(NULL)
  
  repeat{ # 분자들의 좌표가 무조건 겹칠 수밖에 없다면 분자 1부터 다시 새로운 위치를 정해주는 기능
    
    for(j in 1:30){ # 각 분자들이 0.05초 동안 움직여서 나온 새로운 좌표를 기존의 데이터 프레임에 추가해주는 기능 (분자는 총 30개)
